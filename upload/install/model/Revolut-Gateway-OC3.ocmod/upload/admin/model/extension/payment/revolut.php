...
class ModelExtensionPaymentRevolut extends Model {
    ...
    public function updateWebhook($api_key) {
        if ($this->config->get('payment_revolut_api_key') != $api_key) {
            $this->deleteWebhook();
            $this->setWebhookId();
        }
    }
    ...
    public function deleteWebhook() {
        if ($this->config->get('payment_revolut_webhook_id')) {
            $api_request = new ApiRequest($this->config->get('payment_revolut_api_key'), $this->config->get('payment_revolut_test'));

            $result = $api_request->delete('webhooks/' . $this->config->get('payment_revolut_webhook_id'));
    ...
    private function setWebhookId($webhook_id = '') {
        $this->db->query("DELETE FROM `" . DB_PREFIX . "setting` WHERE store_id = '0' AND `code` = 'payment_revolut_webhook' AND `key` = 'payment_revolut_webhook_id'");

        if ($webhook_id) {
            $this->db->query("INSERT INTO `" . DB_PREFIX . "setting` SET store_id = '0', `code` = 'payment_revolut_webhook', `key` = 'payment_revolut_webhook_id', `value` = '" . $this->db->escape($webhook_id) . "'");
        }
    }
}