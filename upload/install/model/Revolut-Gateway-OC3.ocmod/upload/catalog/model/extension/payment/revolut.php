...
class ModelExtensionPaymentRevolut extends Model {
    public function getMethod($address, $total) {
        ...
        if ($status) {
            $method_data = array(
                'code'       => 'revolut',
                'title'      => ($this->config->get('payment_revolut_payment_title') ? $this->config->get('payment_revolut_payment_title') : $this->language->get('text_title')) . '<img src="' . HTTPS_SERVER . 'image/payment/revolut/visa_mc.png' . '" title="Visa/Mastercard" alt="Visa/Mastercard" class="img-responsive" />',
                'terms'      => '',
                'sort_order' => $this->config->get('payment_revolut_sort_order')
            );
        }
