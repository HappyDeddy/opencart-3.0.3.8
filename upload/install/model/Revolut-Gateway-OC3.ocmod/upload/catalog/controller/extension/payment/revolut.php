...
class ControllerExtensionPaymentRevolut extends Controller {
    public function index() {
        ...
        if ($this->config->get('payment_revolut_customise') && $this->config->get('payment_revolut_background_colour')) {
            $data['background_colour'] = $this->config->get('payment_revolut_background_colour');
        } else {
            $data['background_colour'] = '#ffffff';
        }

        if ($this->config->get('payment_revolut_customise') && $this->config->get('payment_revolut_font_colour')) {
            $data['font_colour'] = $this->config->get('payment_revolut_font_colour');
        } else {
        ...
        if ($order_info) {
            ...
            $body = array(
                ...
                'capture_mode'           => $this->config->get('payment_revolut_capture_mode'),
                ...
                if ($order_info && $revolut_order_info) {
                    switch ($data['event']) {
                        ...
                        case 'ORDER_PAYMENT_DECLINED':
                            $status = 'CANCELLED';
                            $order_status_id = $this->config->get('payment_revolut_cancelled_status_id');
        ...
        if (!empty($this->session->data['order_id']) && !empty($this->request->post['revolut_id'])) {
            ...
            if ($order_info && $revolut_order_info) {
                ...
                for ($i = 1; $i <= 9; $i++) {
                    ...
                    if ($http_code == '200') {
                        if ($response['state'] == 'COMPLETED' && $this->config->get('payment_revolut_capture_mode') == 'AUTOMATIC') {
                            $order_status_id = $this->config->get('payment_revolut_completed_status_id');
                            ...
                            break;
                        } elseif ($response['state'] == 'AUTHORISED' && $this->config->get('payment_revolut_capture_mode') == 'MANUAL') {
                            $order_status_id = $this->config->get('payment_revolut_authorised_status_id');
    ...
    public function eventPostModelAddOrderHistory($route, &$args) {
        ...
        if ($order_info && $order_info['order_status_id'] && $order_status_id && $this->config->get('payment_revolut_capture_mode') == 'MANUAL' && $order_status_id == $this->config->get('payment_revolut_capture_status_id') && !isset($this->session->data['revolut_capture'])) {
            $this->load->model('extension/payment/revolut');
            ...
            if ($revolut_order_info) {
                ...
                if ($http_code == '200' && $response['state'] == 'COMPLETED') {
                    ...
                    $order_history_info = $this->model_extension_payment_revolut->getOrderHistory($order_id, $this->config->get('payment_revolut_capture_status_id'), $comment);
    ...
    private function getSVGImage() {
        if ($this->config->get('payment_revolut_customise') && $this->config->get('payment_revolut_logo_theme')) {
            $logo_colour = $this->config->get('payment_revolut_logo_theme');
        } else {
            $logo_colour = '#7a7a7a';
        }
