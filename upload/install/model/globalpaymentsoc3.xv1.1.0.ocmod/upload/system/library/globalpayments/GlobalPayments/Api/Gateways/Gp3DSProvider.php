...
class Gp3DSProvider extends RestGateway implements ISecure3dProvider
{
    ...
    public function processSecure3d(Secure3dBuilder $builder)
    {
        ...
        if ($transType === TransactionType::VERIFY_ENROLLED) {
            ...
            $request['order'] = $this->maybeSetKey($request['order'], 'date_time_created', (new \DateTime($builder->getOrderCreateDate()))->format('Y-m-d\TH:i:s.z\Z'));
            ...
            // payer
            $request['payer'] = [];
            $request['payer'] = $this->maybeSetKey($request['payer'], 'email', null !== $builder->getCustomerEmail() ? $builder->getCustomerEmail() : null);
    ...
    private function mapResponse($rawResponse)
    {
        ...
        $secureEcom->issuerAcsUrl = (isset($doc['method_url']) ? $doc['method_url'] : null) . (isset($doc['challenge_request_url']) ? $doc['challenge_request_url'] : null);
        ...
        if (array_key_exists('message_extension', $doc)) {
            $secureEcom->criticalityIndicator = isset($doc['message_extension']['criticality_indicator']) ? $doc['message_extension']['criticality_indicator'] : null;
            $secureEcom->messageExtensionId = isset($doc['message_extension']['id']) ? $doc['message_extension']['id'] : null;
            $secureEcom->messageExtensionName = isset($doc['message_extension']['name']) ? $doc['message_extension']['name'] : null;
        }

        // versions
        $secureEcom->directoryServerEndVersion = isset($doc['ds_protocol_version_end']) ? $doc['ds_protocol_version_end'] : null;
        $secureEcom->directoryServerStartVersion = isset($doc['ds_protocol_version_start']) ? $doc['ds_protocol_version_start'] : null;
        $secureEcom->acsEndVersion = isset($doc['acs_protocol_version_end']) ? $doc['acs_protocol_version_end'] : null;
        $secureEcom->acsStartVersion = isset($doc['acs_protocol_version_start']) ? $doc['acs_protocol_version_start'] : null;

        // payer authentication request
        if (array_key_exists('method_data', $doc)) {
            $methodData = $doc['method_data'];
            $secureEcom->payerAuthenticationRequest = isset($methodData['encoded_method_data']) ? $methodData['encoded_method_data'] : null;
