...
var RealexRemote = (function() {
	...
	var internal = {
		...
		isMessageFromAcs: function (origin, acsUrl) {
			return internal.getHostnameFromUrl(origin) === internal.getHostnameFromUrl(acsUrl);
		},

		receiveMessage: function (lightboxInstance, merchantUrl, idOfLightboxForm, isEmbedded) {
			return function (event) {			
				...
				if (event.data && JSON.parse(event.data).iframe) {
					...
					if (isMobileNewTab && tabWindow) {
						...
						var spinner = document.getElementById("rxp-loader-" + randomId);
						var overlay = document.getElementById("rxp-overlay-" + randomId);
						
						lightboxInstance.close(closeButton, iFrame, spinner, overlay);
					}

					var authenticationData = JSON.parse(event.data);
					
					showGlobalPaymentsAlert(authenticationData);
																				
					$.ajax({
						url: merchantUrl,
						type: 'post',
						data: $('#' + idOfLightboxForm).serialize() + '&authenticationData=' + JSON.stringify(authenticationData),
						dataType: 'json',
						async: false,
	...
	var RxpLightbox = (function () {
		...
		function init() {
			...
			return {
				...
				close: function (closeButton, iFrame, spinner, overlayElement) {
					internal.closeModal(closeButton, iFrame, spinner, overlayElement);
				},
				setToken: function (acsToken) {
					token = acsToken;
				}
			};
		}
		...
		return {
			...
			getInstance: function (acsToken) {
				if (!instance) {
					instance = init();
				}

				//Set the acs token
				instance.setToken(acsToken);

				return instance;
			},
			init: function (idOfLightboxForm, merchantUrl, serverSdkJson) {
				...
				if (window.addEventListener) {
					window.addEventListener("message", internal.receiveMessage(lightboxInstance, merchantUrl, idOfLightboxForm), false);
				} else {
					window.attachEvent('message', internal.receiveMessage(lightboxInstance, merchantUrl, idOfLightboxForm));
				}
